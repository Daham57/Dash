    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100">
      <div className="px-8 py-6 border-b border-gray-100">
        <h3 className="text-xl font-semibold text-gray-900">
          {initialData ? t('courses.editCourse') : t('courses.createNewCourse')}
        </h3>
        <p className="text-sm text-gray-600 mt-1">
          {initialData
            ? t('courses.updateCourseDetails')
            : t('courses.fillCourseDetails')}
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-8 space-y-10">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            />
            <Input
              label={t('courses.courseStartTime')}
              type="time"
              value={formData.course_start_time}
              onChange={(e) =>
                handleChange("course_start_time", e.target.value)
              }
              required
            />
            <Input
            ...
            />
            <Input
              label={t('courses.expectedEndDate')}
              type="date"
              value={formData.expected_end_date}
              onChange={(e) =>
                handleChange("expected_end_date", e.target.value)
              }
              required
            />
          </div>
        ...
        <div className="space-y-6">
          <h4 className="text-lg font-medium text-gray-900 border-b border-gray-200 pb-2">
            {t('courses.mediaFiles')}
          </h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <FileUpload
              label={t('courses.courseImage')}
              value={imageFile}
              onChange={setImageFile}
              helperText={t('courses.uploadImage')}
              accept="image/*"
            />
            <FileUpload
              label={t('courses.courseFile')}
              value={pdfFile}
              onChange={setPdfFile}
              helperText={t('courses.uploadFile')}
              accept=".pdf"
            />
            <Input
              label={t('courses.fileName')}
              value={fileName}
        ...
        <div className="space-y-6">
          <h4 className="text-lg font-medium text-gray-900 border-b border-gray-200 pb-2">
            {t('courses.courseParticipants')}
          </h4>

          {/* Instructors */}
          <div>
            <label className="block text-sm font-semibold text-gray-800 mb-4">
              {t('courses.selectInstructors')}
            </label>
