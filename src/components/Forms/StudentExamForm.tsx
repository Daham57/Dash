    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100">
      <div className="px-8 py-6 border-b border-gray-100">
        <h3 className="text-xl font-semibold text-gray-900">
          {initialData ? t('studentExams.editStudentExamRecord') : t('studentExams.addNewStudentExam')}
        </h3>
        <p className="text-sm text-gray-600 mt-1">
          {initialData 
            ? t('studentExams.updateStudentExamDetails')
            : t('studentExams.fillStudentExamDetails')
          }
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-8 space-y-8">
        ...
        <div className="space-y-6">
          <h4 className="text-lg font-medium text-gray-900 border-b border-gray-200 pb-2">
            {t('studentExams.examStudentInfo')}
          </h4>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Select
              label={t('studentExams.exam')}
            ...
            <Select
              label={selectedExam && courseStudents.length > 0 ? t('studentExams.courseStudents') : t('studentExams.allStudents')}
          ...
          {selectedExam && courseStudents.length > 0 && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <p className="text-sm text-blue-800">
                <strong>{t('studentExams.note')}:</strong> {t('studentExams.noteOnlyEnrolled')}
              </p>
            </div>
          )}
        </div>
        ...
        {selectedExamData && (
          <div className="space-y-6">
            ...
            <div className="bg-gray-50 rounded-lg p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <span className="text-sm font-medium text-gray-600">{t('studentExams.maxMark')}:</span>
                <p className="text-lg font-bold text-blue-600">{(selectedExamData as any).max_mark || 0}</p>
              </div>
              <div>
                <span className="text-sm font-medium text-gray-600">{t('studentExams.passingMark')}:</span>
                <p className="text-lg font-bold text-yellow-600">{(selectedExamData as any).passing_mark || 0}</p>
              </div>
              <div>
                <span className="text-sm font-medium text-gray-600">{t('studentExams.courseName')}:</span>
        ...
        <div className="space-y-6">
          <h4 className="text-lg font-medium text-gray-900 border-b border-gray-200 pb-2">
            {t('studentExams.studentPerformance')}
          </h4>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Input
              ...
              helperText={selectedExamData ? t('studentExams.maximumPossibleMark') + `: ${(selectedExamData as any).max_mark}` : undefined}
              required
            />
              ...
              <div className="flex items-center justify-center">
                <div className={`text-center p-4 rounded-lg ${
                  ...
                  <p className={`text-lg font-bold ${
                    ...
                    {formData.student_mark >= ((selectedExamData as any).passing_mark || 0) ? t('studentExams.pass') : t('studentExams.fail')}
                  </p>
        ...
        <div className="flex justify-end space-x-4 pt-6 border-t border-gray-200">
          ...
          <Button type="submit" loading={loading} size="lg">
            {initialData ? t('studentExams.updateStudentExam') : t('studentExams.createStudentExam')}
          </Button>
        </div>
      </form>
    </div>
