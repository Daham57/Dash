...
export const AttendanceManagement: React.FC = () => {
  ...
  const [lessonsWithoutAttendance, setLessonsWithoutAttendance] = useState<
    Lesson[]
  >([]);
      ...
      if (res.data.attendance && Array.isArray(res.data.attendance)) {
        ...
        const validAttendance = res.data.attendance.filter((record: any) => {
          if (record.student?.qr_code) {
            ...
            if (!isValid) {
              console.warn(
                `Invalid QR code for student ${record.student.name}`
              );
            }
            return isValid;
          }
        ...
        // Success message
        setSessionMessage(
          `${res.data.message} (${res.data.count || 0} ${t(
            "messages.studentsEnrolled",
        ...
        const invalidCount =
          res.data.attendance.length - validAttendance.length;
        if (invalidCount > 0) {
          setErrorMessage(
            t("messages.invalidQrWarning", { count: invalidCount })
          );
        }
      }
...
const handleQRScan = async (qrData: string) => {
  ...
  try {
    // نبحث عن سجل الحضور الخاص بالطالب والدرس المحدد
    const attendanceRecord = attendance.find(
      (a) => a.student_id === currentStudentId && a.lesson_id === selectedLesson
    );

    if (attendanceRecord) {
      // تحديث الحضور بإرسال lesson_id في الرابط
      // وبيانات الطالب + حالة الحضور + وقت التحديث + QR في الجسم
      await apiService.update("atten", selectedLesson, {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_value: any, record: Attendance) => {
        const hasValidQR =
          record.student?.qr_code && validateQRCode(record.student.qr_code);
        return (
          <span
            className={`px-2 py-1 rounded-full text-xs font-medium ${
              hasValidQR
    ...
    <div className="p-6 space-y-6">
      ...
      {!loading && lessonsWithoutAttendance.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500">{t("attendance.allLessonsMessage")}</p>
        </div>
      )}
